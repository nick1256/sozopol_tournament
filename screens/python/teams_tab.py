...
class TeamsView(GridLayout):
	...
	def select_category_button(self, category):
		""" show which category button is selected """
		self.left_side.select_button(category)

	def select_team_button(self, team):
		""" show which team button is selected """
...
class TeamsButtons(GridLayout):
	...
	def __init__(self, **kwargs):
		...
		self.buttons["Small"] = Button(text="Teams 6-7", size_hint_y=None, height=ROW_HEIGHT,
                                 on_press=(lambda x: self.parent.show_category("Small")))

		self.buttons["Medium"] = Button(text="Teams 8-9", size_hint_y=None, height=ROW_HEIGHT,
                                  on_press=(lambda x: self.parent.show_category("Medium")))

		self.buttons["Big"] = Button(text="Teams 10-12", size_hint_y=None, height=ROW_HEIGHT,
                               on_press=(lambda x: self.parent.show_category("Big")))

		# draw widgets
		for button_text in sorted(self.buttons, reverse=True):
			self.add_widget(self.buttons[button_text])

	def select_button(self, category):

		""" press a button and highlight it """
...
class TeamsSelection(GridLayout):
	...
	def show_category(self, category):
		...
		# draw widgets
		for button_text in sorted(self.buttons):
			self.add_widget(self.buttons[button_text])
...
class PeopleView(GridLayout):
	...
	def show_team(self, team):
		...
		query = """select name, alg_scores, comb_scores, geom_scores, num_scores, total_scores
				from People_{} where team=\"{}\" order by name;""".format(category, team)
	...
	def save(self):
		...
		cursor.execute("""delete from People_{} where
			team=\"{}\";""".format(self.parent.category, self.parent.curr_team))
...
class PeopleRow(GridLayout):
	...
	def __init__(self, loaded=None, **kwargs):
		...
		# add widgets
		if not loaded:
			self.widgets.append(TextInput(size_hint_x=0.5, multiline=False,
                                 on_text_validate=(lambda x: self.commit_person())))
...
class PeopleButtons(GridLayout):
	...
	def __init__(self, **kwargs):
		...
		self.cols = 2
		self.spacing = 50
		self.add_widget(Button(text="Add Person", size_hint=(None, None), height=ROW_HEIGHT,
                         on_press=(lambda x: self.add_person())))

		self.add_widget(Button(text="Save Changes", size_hint=(None, None),
                         height=ROW_HEIGHT, on_press=(lambda x: self.save())))
