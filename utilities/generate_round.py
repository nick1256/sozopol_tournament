...
def backtrack_search(team_names, domains):
	"""backtrack searching, call with empty assignment"""
	return backtrack([], team_names, domains)

def backtrack(assignment, team_names, domains):
	...
	for second_team in team_names:
		...
		if second_team not in domains[first_team]:
			continue
		...
		result = backtrack(new_assignment, new_team_names, domains)
...
def generate_matches(cursor, category):

	"""splitting teams into matches"""

	# get teams sorted according to scores
	query = """select visible_name,rank_scores,points_diff,points from Teams_{}
			order by rank_scores desc, points_diff desc".format(category)"""
	...
	if nteams%2 == 1:
		...
		min_sc = min([team[1] for team in teams_info if team not in free_win_teams])

		# primitive randomizing
		lowest = set([team for team in teams_info if team[1] == min_sc])
	...
	for team_name in team_names:
		domains[team_name] = [team for team in team_names if team not in forbidden_matches[team_name]]
	...
	while counter != nmatches and alone_jury:
	...
	if counter == nmatches:
		counter -= 1
		while counter != -1 and alone_jury:
			name = alone_jury.pop(0)
			matches[counter].append(name)
			counter -= 1

		while counter != -1 and extra_jury:
			name = extra_jury.pop(0)
			matches[counter].append(name)
			counter -= 1

	else:
		while counter < nmatches and extra_jury:
			name = extra_jury.pop(0)
			matches[counter].append(name)
			counter += 1
